...
dbapi.checkUserBeforeLogin = function(user) {
  return db.run(r.table('users').get(user.email)).then(function(oldUser) {
    // For mock auth, allow login with any name as long as email exists
    if (!oldUser) {
      return false
    }
    // Always return true if user exists (ignore name mismatch for mock auth)
    return true
  })
}
...
dbapi.loadDevices = function(groups) {
  return db.run(r.table('devices').filter(function(device) {
    // TODO: Original code didn't handle '*' wildcard for admin users
    // OLD: return r.expr(groups).contains(device('group')('id'))
    // FIX: Support '*' wildcard to match all devices for admin users
    // This allows users with subscribed: ['*'] to see all devices
    return r.expr(groups).contains('*').or(r.expr(groups).contains(device('group')('id')))
  }))
  .then(function(cursor) {
    return cursor.toArray()
  })
...
dbapi.loadDevicesByOrigin = function(groups) {
  return db.run(r.table('devices').filter(function(device) {
    // TODO: Original code didn't handle '*' wildcard for origin groups
    // OLD: return r.expr(groups).contains(device('group')('origin'))
    // FIX: Support '*' wildcard for origin group filtering
    return r.expr(groups).contains('*').or(r.expr(groups).contains(device('group')('origin')))
  }))
  .then(function(cursor) {
    return cursor.toArray()
  })
...
dbapi.loadBookableDevices = function(groups) {
  return db.run(r.table('devices').filter(function(device) {
    // TODO: Original bookable devices query didn't support admin wildcard
    // OLD: return r.expr(groups).contains(device('group')('origin')).and(device('group')('class').ne(apiutil.STANDARD))
    // FIX: Support '*' wildcard for bookable devices (non-standard class)
    return r.expr(groups).contains('*')
      .or(r.expr(groups).contains(device('group')('origin')))
...
dbapi.loadStandardDevices = function(groups) {
  return db.run(r.table('devices').filter(function(device) {
    // TODO: Original standard devices query didn't support admin wildcard
    // OLD: return r.expr(groups).contains(device('group')('origin')).and(device('group')('class').eq(apiutil.STANDARD))
    // FIX: Support '*' wildcard for standard devices (standard class only)
    return r.expr(groups).contains('*')
      .or(r.expr(groups).contains(device('group')('origin')))
...
dbapi.loadDevice = function(groups, serial) {
  return db.run(r.table('devices').getAll(serial).filter(function(device) {
    // TODO: Original loadDevice didn't handle '*' wildcard for admin users
    // OLD: return r.expr(groups).contains(device('group')('id'))
    // FIX: Support '*' wildcard to match device for admin users
    return r.expr(groups).contains('*').or(r.expr(groups).contains(device('group')('id')))
  }))
}
